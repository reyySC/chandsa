...
![Shreya Chandra's profile site live at reyysc.github.io](screenshots/01-homepage.png)

*Screenshot 1 — Shreya Chandra's profile site served from https://reyysc.github.io, on a first visit.*

![The waph.html course page](screenshots/02-waph-page.png)

*Screenshot 2 — the `waph.html` course page at https://reyysc.github.io/waph.html, listing Dr. Phu Phung as
instructor and Shreya Chandra as student.*
...
![The footer page tracker](screenshots/03-footer-tracker.png)

*Screenshot 3 — the footer of Shreya Chandra's site: the cookie-backed visit counter and the Flag Counter
widget. The image panel above shows the Dog CEO fallback, which is what renders when the NASA APOD request
fails.*
...
![Both clocks running and the email revealed](screenshots/04-clocks-and-email.png)

*Screenshot 4 — Shreya Chandra's header with the jQuery digital clock (20:20:25), the canvas analog clock,
and the email address revealed by the show/hide control.*

![The Vue skills filter in use](screenshots/05-skills-filter.png)

*Screenshot 5 — Shreya Chandra's skills filter: typing `pyt` narrows 33 chips to 2 (Python, PyTorch) and the
...
![The two public API integrations](screenshots/06-api-section.png)

*Screenshot 6 — the two public web APIs on Shreya Chandra's page: a JokeAPI joke and the NASA Astronomy
Picture of the Day, with the third-party disclaimer below them.*
...
![First-visit greeting](screenshots/07-first-visit.png)

*Screenshot 7 — first visit to Shreya Chandra's site with no `lastVisit` cookie set: the status bar reads
"Welcome to my homepage for the first time!".*

![Return-visit greeting](screenshots/08-return-visit.png)

*Screenshot 8 — the same browser after a reload: "Welcome back! Your last visit was 20/08/2026, 20:22:05",
read back from the cookie.*

![The cookies in DevTools](screenshots/09-cookies-devtools.png)

*Screenshot 9 — DevTools → Application → Cookies for reyysc.github.io, showing `lastVisit`, `visitCount`
and `theme` with their values and expiry dates.*

The expiry column is worth a note. The code requests a 365-day lifetime, but the panel reports these cookies
expiring on 2027-02-16 — roughly 180 days out. Brave caps the lifetime of cookies written by JavaScript, so
the browser, not the page, has the last word on how long client-side state survives.

![Dark mode](screenshots/10-dark-mode.png)

*Screenshot 10 — Shreya Chandra's site in dark mode, the extra feature of my choice; the preference is
stored in a cookie and survives a reload.*
